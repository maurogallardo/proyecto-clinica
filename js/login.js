...
// Después del login: por ahora, la muestra de la T045 (hasta que exista la T013)
async function entrarALaApp() {
  const sesion = await Sesion.actual();
  const correo = document.getElementById('muestra-correo');
  if (correo && sesion) correo.textContent = sesion.user.email;
  mostrarPantalla(PANTALLA_PRINCIPAL);
}

// Botón provisorio "Cerrar sesión" de la muestra
function prepararCerrarSesion() {
  const boton = document.getElementById('muestra-cerrar-sesion');
  if (!boton) return;
